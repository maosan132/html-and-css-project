...
const orderedMedals = (country, field, sortAscending) => {
  return `SELECT * \
          FROM GoldMedal \
          WHERE country = '${country}'\
          ORDER BY ${field ? field : '1'} ${sortAscending ? 'ASC' : 'DESC'}`;
};
...
const orderedSports = (country, field, sortAscending) => {
  return `WITH MedalsByCountry AS (\
            SELECT country, COUNT(*) AS total\
            FROM GoldMedal\
            WHERE country = '${country}'\
            GROUP BY country\
          )\
          SELECT GoldMedal.sport, COUNT(*) AS 'count',  (COUNT(*) * 100 / total) AS 'percent'\
          FROM GoldMedal \
          JOIN MedalsByCountry ON MedalsByCountry.country = GoldMedal.country\
          WHERE GoldMedal.country = '${country}'\
          GROUP BY GoldMedal.sport\
          ORDER BY ${field ? field : '1'} ${sortAscending ? 'ASC' : 'DESC'}`;
};
